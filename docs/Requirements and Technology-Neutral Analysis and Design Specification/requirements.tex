...
\subsection{Reliability Tactics}
	...
	\subsubsection{In context of the system :} 
		\begin{itemize}
			...
			\item The system must resolve synchronization conflicts using timestamps.
				\subparagraph 
					- The Ping/Echo Architectural tactic will be used to achieve this requirement. 
			...
			\item Hot swapping of system modules should not affect system service reliability.
				\subparagraph
					- The Broker Archictectural Pattern will be used here because it maintains communication with the server. Since it adds timing behaviour, it can be used to roll back events to repaire the system should it be affected by hot swapping of system modules. 
					- The system will not suffer much strain since it is designed using the MVC model, only some modules would be affected but detection and repair is made easy because the system will be made up of separate individual modules. 
...
\subsection{Scalability Tactics}
	...
	\subsubsection{In context of the system :}
		\begin{itemize}
			...
			\item The system should be able to facilitate 50 concurrent user sessions at any time.
				\subparagraph
					- Centralized control leads to resource bottlenecks, which is not favoured favoured especially since 50 concurrent users should be able to use the system concurrently.  
					- Decentralising control to allow multiple users to act on the system at a time, more especially if their processes are not executing critical regions is an Architectural tactic which will allow multiple users to act independently on the system. 
		\end{itemize}
...
\subsection{Maintainability Tactics}
	...
	\subparagraph
		...
		- Preventative : preventing future problems
...
\subsubsection{In context of the system :} 
		\begin{itemize}
			...
			\item The back-end system should integrate with a desktop web client and Android mobile app clients.
				\subparagraph
					- This requirement will be fulfilled through the use of Hybrid. Hybrid has two main components, native code (Java and Android frameworks) and web code (HTML/CSS/JavaScript). This will give us a web interface which can easily be bundled as an Android Application. 
		\end{itemize}
