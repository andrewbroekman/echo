...
\subsection{Performance Tactics}
	\subsubsection{Performance Tactics can be divided into 3 groups}
		\begin{enumerate}
			\item Tactics for managing resource demand : these tactics contol execution and transfer times by controlling the allocation of resources
			\item Tactics for managing conflict resolution : these tactics control premeption and waiting times when competing requests execute concurrently. 
...
\subsection{Reliability Tactics}
	...
	\subsubsection{In context of the system :} 
		\begin{itemize}
			\item To enable offline activity and access of data from the Android app, data synced must be made differentiable with timestamps."
				\subparagraph
					- This will be achieved by providing persistent data storage services with state information, ensuring that the results obtained - either using the Android application or the website is still valid.  
					- A variation of the Blackboard Architectural pattern will be used to define checkpoints which allow for roll back functionality. All inputs into the system will be logged in order, in this way, should an inconsistency occur, one could reconstruct the sequence of events. 
			\item The system must resolve synchronization conflicts using timestamps.
				\subparagraph 
					- The Ping/Echo Achitectural tactic will be used to achieve this requirement. 
					- It has two major components ; the one issuing the ping and keeping time and the components responding to the ping with an echo. 
					- The sequence of actions is controlled by a clock, independent of all the other actions in the system. 
					- Another way this requirement will be fulfilled is through the use synchronization locks (which ensure that only one process at a time can make use of a shared resource) on all critical sections of the system, processes must attempt to acquire a lock before they can use a shared resource and release the lock once they are done using the resource.
					-Voting tactic will be used for conflict resolution here : different components work on the same problem, their results are compared and the voting scheme is used in cases where results differ.
			\item Hot swapping of system modules should not affect system service reliability.
				\subparagraph
					- The Broker Achictectural Pattern will be used here because it maintains communication with the server. Since it adds timing behaviour, it can be used to roll back events to repaire the system should it be affected by hot swapping of system modules. 
					- The system will not suffer much strain since it is designed using the MVC model, only some modules would be affected but detection and repaire is made easy because the system will be made up of seperate individual modules. 
					- Hiding information : modules of the system will have their responsibilities divided into two categories - public and private. This will limit responsibility as well as dependency so one module failing will not affect the others by a huge margin. 
		\end{itemize}
...
\subsection{Scalability Tactics}
	\subsubsection{Definition of Scalability}
		...
		- One way to achieve scalability is to perform a regualar SWOT analysis on the system: 
	...
	\subsubsection{In context of the system :}
		\begin{itemize}
			\item The system should be able to handle 10 research groups with each research group containing 100 members.
				\subparagraph
					- Developers need to be aware of the systems abilities at all times, this will be achieved through regular Scalability Audits. 
					- If demand never reaches a critical level, the incremental cost for scalability will be wasted because designing a system to be scalable may entail upfront costs that are higher than those of a less scalable version of the same system. 
			\item The system should be able to facilitate 50 concurrent user sessions at any time.
				\subparagraph
					- Centralized control leads to resource bottlenecks, which is unfavoured especially since 50 concurrent users should be able to use the system concurrently.  
					- Decentralising control to allow multiple users to act on the system at a time, more especially if their processes are not executing critical regions is an Achitectural tactic which will allow multiple users to act independently on the system. 
		\end{itemize}
...
\subsection{Security Tactics} 
	...
	\subsubsection{In context of the system :} 
		\begin{itemize}
			\item System should be resistant to SQL injections.
				\subparagraph
					- Encrypting data is one measure which will be used to prevent SQL injections. SQL injections rely on knowing what type of data is expected, if this data is encrypted when it gets to server and turned into something only the system will understand, then a SQL injection attack can be avoided. 
					-The system never stores authentication information as simple text. 
			\item A user group hierarchy system should be used to manage user access rights.
				\subparagraph
					- The different types of actors on the system will have different access rights, this will be accomplished by disabling some functionality and modules depending on the level of user that logged in. 
			\item Authentication credentials such as username and password should not be stored on Android user device. A token based authentication approach should be utilized so that only tokens are stored on end user device, allowing for the easy revocation of device access if a device is lost. Available technologies, such as OAuth, should be utilized.
				\subparagraph
					- OAuth will be used to achieve this requirement. 
					- OAuth is an authorization framework which enables a third-party application to obtain limited access to an HTTP service. 
					- OAuth is serves as a simple mechanism to publish and interact with protected data. 
			\item Password hosting with a unique salt for each user should be used.
				\subparagraph
			...
			\item A key derivation function should be used with passwords.
				\subparagraph
					- This functionality will happen during hashing of the password before it is stored persistently. 
			\item Database access must require authentication.
				\subparagraph
					- The level of access and ability to view or modify any database linked to the system will be determined by the type of user logged into the system. 
					- User authentication will be performed using Access Tokens, which will be checked for validity every time a database is queried. 
		\end{itemize}
...
\subsection{Flexibility Tactics}
	...
	\subsection{In context of the system :}
		\begin{itemize}
			...
			\item The system should be decoupled from the database technology it uses and allow the client to select and change the database it uses in future.
				\subparagraph
					- This functionality will be achived by converting database tables to XML. As long the structure and data of the database is maintained, the XML version can be converted back into a database - this allows the information to be preserved in the new database too. 
			...
			\item Authentication mechanisms used should be decoupled from the system, allowing them to be interchangeable.
				\subparagraph
					-This quality requiremet will be achieved using The Pluggable Authentication Modules(PAM) library, which is a generalised API for authentiction-related services. This API allows the system developer to add new authentication methods by simply installing new PAM modules. This will give the system the ability to add and remove authenication mechanisms. 
		\end{itemize}
...
\subsection{Maintainability Tactics}
	...
	\subparagraph
		There are four categories of Software Maintainance
		- Adaptive : changes in the software environment 
		- Perfective : new user requirements 
		- Corrective : fixing errors 
		- Preventitive : preventing future problems

	\subsubsection{In context of the system :} 
		\begin{itemize}
			\item All code should be documented in the applicable language documentation framework, such as JavaDocs for a Java based system, DOxygen for a C/C++ based system, etc.
				\subparagraph
					- Doxygen will be used for code documentation since it supports a variety of programming languages such as Java, C\#, and PHP. 
			\item A coding style guide/manual should be set up and associated with the project, such that all developers use similar coding styles and conventions, to allow for more readable code that is easier to maintain.
				\subparagraph
					- Simple good coding practices known by all system developers should be used. These include camel casing and giving descriptive names to variables, functions and classes. 
			\item System should be separated in distinct, concise and independent modules relating to separate concerns, to allow for easier maintenance.
				\subparagraph
					- The Spring framework is used for this quality requirement because it supports seperating the different modules of the system so that they can be tested as independent modules. 
					- Spring also supports transaction management which unifies several transaction management APIs and co-ordinates transactions for Java objects.
		\end{itemize}
...
\subsection{Auditability Tactics}
	...
	\subsubsection{In context of the system :}
		\begin{itemize}
			\item Data in database should always be consistent. This implies that all data should adhere to constraints placed on the data by the data model, such as regex patterns, minimum and maximum length, non-nullable fields, etc.
				\subparagraph
					-  Data validation will be enforced by the developer on the client side, this will ensure that only valid data will be entered into any database associated with the system. 
 					- RegEx(Regular Expressions) Patterns will be used to ensure that any input into the system is of the right format. 
 			\item No data should ever be deleted.
 				\subparagraph
				 - This will be accomplished by not adding any functionality that gives regular users to remove records from the system. The only way to 'delete' information from the system is to render it unusable - by disabling access to it - a feature which must be enforced by the developer.
			\item The rest of the quality requirements pertain to logging of system actions.
				\subparagraph
				- This requirement will be succeeded by externally logging all inputs and actions by the user by having the system write them to a text file. 
				- Logging system data will not be a user function. Logging happens automatically by the system, without the user being aware or able to intercept the process.
		\end{itemize}

\subsection{Integrability and Extensibility Tactics}
	\subparagraph
		- Integrability evealuation refers to testing whether seperately developed components work together correctly.  Extensibility evaluation focuses on how new features can be added without losing existing features

\subsubsection{In context of the system :} 
		\begin{itemize}
			\item The system should allow technology neutral importing and exporting of data.
				\subparagraph
					- The design of the database will allow the information stored to be easily converted to XML format, this makes it easy to export the information stored in the tables to be used by another technology. 
			\item The back-end system should integrate with a desktop web client and Android mobile app clients.
				\subparagraph
					- This requirment will be fulfilled through the use of Hybrid. Hybrid has two main components, native code (Java and Android frameworks) and web code (HTML/CSS/JavaScript). This will give us a web interface which can easily be bundled as an Android Application. 
		\end{itemize}
...
\subsection{Cost Tactics} 
	\subsubsection{In context of the system :}
		\begin{itemize}
			\item System should be cheap to operate, maintain and extend. If the quality and maturity of technologies available allow it, technologies used must be freely available/usable.
				\subparagraph	
					- As it is, no special technology is required by the user to interact with the system. The user only needs a device to access the system. 
			\item As far as possible, open source compatible, mature technologies should be used, to ensure system stability as far as possible.
				\subparagraph
					- Free Open Source software will be used to build the entire system. The only thing required when writing web pages is a text editor and a web browser to view the pages. NetBeans is a free IDE which can easily be acquired and used without a product key. 
		\end{itemize}
...
\subsection{Usability Tactics}
	\subsection{In context of the system :}
		\begin{itemize}
			\item The web client should render properly and be fully functional in modern web browsers.
				\subparagraph
					- One way the system will achieve the optimization of web page rendering is that while running javascript code, browsers cache the changes and apply them in a single pass after the code was run. 
					- Always use the (\#id) selector for CSS because it is the fastest and will optimize rendering of the Cascading Style Sheets. 
			\item Mobile devices running Android 4.2 and upwards should be fully supported.
			\item Android app and web client should support the full back-end API specifications.
				\subparagraph
					- This quality requirement will be taken care of automatically by Hybrid(as mentioned above.
		\end{itemize}
