...
\subsubsection{Programming Languages}
	\begin{itemize}
		\item HTML\\
			As we will be building a browser based SPA, we will require the use of HTML to deliver and bootstrap the application on the client side.
		\item JavaScript\\
			JavaScript will make out the core of the SPA, in which we will be using AngularJS as the framework for building the applications.
		\item CSS\\
			As we will be using web browsers to render and contain the SPA, we will be using CSS to implement styling in the application.
	\end{itemize}
...
\subsubsection{Dependency Management and Build Tools}
	\begin{itemize}
		\item Gradle\\
			Gradle is an open source build tool based on the Groovy JVM language and builds upon various concepts of Apache Ant and Apache Maven also supporting dependency management.

			As Gradle is the default build tool used with Android applications, we will be using Gradle to build and deploy the Android application for the client.
	\end{itemize}
